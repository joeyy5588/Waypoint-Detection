...
class SubpolicyPreTrainer(Trainer):
    def __init__(self,
        model = None,
        args = None,
        data_collator = None,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        trg_subpolicy = inputs['trg_subpolicy']
        trg_direction = inputs['trg_direction']
        outputs = model(**inputs)
        pred_subpolicy, pred_direction = outputs
        
        # Pretrain waypoint predictor/ view selector
        # ce_loss = nn.CrossEntropyLoss()
        # weight = [0,0,0.05,0.05,0.05,0.05,0.1,0.14,0.14,0.14,0.14,0.14]
        focal_loss = FocalLoss(gamma=2, alpha=None, reduction='mean', device=trg_subpolicy.device)

        
        subpolicy_loss = focal_loss(pred_subpolicy, trg_subpolicy)
        # direction_loss = ce_loss(pred_direction, trg_direction)
        # training_loss = distance_loss*2 + direction_loss
        training_loss = subpolicy_loss
        # training_loss = direction_loss


        loss_metric = {
            "training_loss": training_loss.item(),
            "subpolicy_loss": subpolicy_loss.item(),
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        for i, (inputs) in enumerate(tqdm(eval_dataloader)):
            if len(inputs['trg_direction']) != self.args.train_batch_size:
                continue
            inputs = self._prepare_inputs(inputs)
            with torch.no_grad():
                trg_subpolicy = inputs['trg_subpolicy']
                ...
                outputs = model(**inputs)
                pred_subpolicy, pred_direction = outputs
                data_num += trg_subpolicy.size(0)

                # direction_loss += ce_loss(pred_direction, trg_direction.flatten()).item()
                # label_idx = trg_direction.flatten()
                # prediction = torch.argmax(pred_direction, dim=1)
                # correct = torch.sum(prediction == label_idx)
                # direction_correct += correct.item()

                subpolicy_loss += ce_loss(pred_subpolicy, trg_subpolicy.flatten()).item()
                label_idx = trg_subpolicy.flatten()
                prediction = torch.argmax(pred_subpolicy, dim=1)
                correct = torch.sum(prediction == label_idx)
                subpolicy_correct += correct.item()

        metrics = {
            # f"{metric_key_prefix}_direction_loss": direction_loss / data_num,
            # f"{metric_key_prefix}_direction_acc": direction_correct / data_num,
            f"{metric_key_prefix}_subpolicy_loss": subpolicy_loss / data_num,
            f"{metric_key_prefix}_subpolicy_acc": subpolicy_correct / data_num,
        }
        # print(metrics)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                if len(inputs['trg_direction']) != (args.train_batch_size):
                    break
