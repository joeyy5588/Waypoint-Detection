...
from waynav.trainer.loss import FocalLoss
...
class Low_Level_Trainer(Trainer):
    def __init__(self,
        model = None,
        args = None,
        data_collator = None,
        ...
        self.action_confmat = ConfusionMatrix(task="multiclass", num_classes=4)
        self.boundary_confmat = ConfusionMatrix(task="binary", num_classes=2)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # interaction_labels = inputs['interaction_labels']
        outputs = model(**inputs)
        # pred_action, pred_boundary = outputs
        pred_action = outputs
        
        # Pretrain waypoint predictor/ view selector
        # ce_loss = nn.CrossEntropyLoss()
        # action_focal_loss = FocalLoss(gamma=2, alpha=[0.1,0.35,0.35,0.2], reduction='mean', device=labels.device)
        action_focal_loss = FocalLoss(gamma=2, alpha=None, reduction='mean', device=labels.device)
        # boundary_focal_loss = FocalLoss(gamma=2, alpha=[0.1,0.9], reduction='mean', device=interaction_labels.device)

        
        # subpolicy_loss = focal_loss(pred_subpolicy, trg_subpolicy)
        action_loss = action_focal_loss(pred_action, labels)
        # boundary_loss = boundary_focal_loss(pred_boundary, interaction_labels)
        training_loss = action_loss# + boundary_loss
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        for i, (inputs) in enumerate(tqdm(eval_dataloader)):
            if len(inputs['labels']) != self.args.train_batch_size:
                continue
            inputs = self._prepare_inputs(inputs)
            with torch.no_grad():
                labels = inputs['labels']
                # interaction_labels = inputs['interaction_labels']
                all_action_labels.append(labels.flatten())
                # all_boundary_labels.append(interaction_labels.flatten())
                outputs = model(**inputs)
                # pred_action, pred_boundary = outputs
                pred_action = outputs
                data_num += labels.size(0)

                action_loss += ce_loss(pred_action, labels.flatten()).item()
                label_idx = labels.flatten()
                prediction = torch.argmax(pred_action, dim=1)
                all_action_preds.append(prediction)
                correct = torch.sum(prediction == label_idx)
                action_correct += correct.item()

                # boundary_loss += ce_loss(pred_boundary, interaction_labels.flatten()).item()
                # label_idx = interaction_labels.flatten()
                # prediction = torch.argmax(pred_boundary, dim=1)
                # all_boundary_preds.append(prediction)
                # correct = torch.sum(prediction == label_idx)
                # boundary_correct += correct.item()
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                if len(inputs['labels']) != (args.train_batch_size):
                    break
