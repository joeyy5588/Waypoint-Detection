...
class BartVLNDecoder(BartPretrainedModel):
    ...
    def __init__(self, config, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if embed_tokens is not None:
            self.embed_tokens.weight = embed_tokens.weight
        # self.subpolicy_embedding = nn.Embedding(12, config.d_model, self.padding_idx)
...
class BARTSubPolicyModel(BartPretrainedModel):
    ...
    def __init__(self, config):
        ...
        self.decoder = BartVLNDecoder(config, self.shared)
        self.post_init()
...
class BartForSubpolicyGeneration(BartPretrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = BARTSubPolicyModel(config)
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
        # self.lm_head = nn.Linear(config.d_model, 12, bias=False)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # print(torch.argmax(lm_logits, dim=-1)[0], labels[0])
