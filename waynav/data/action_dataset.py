...
class Action_Dataset:
            ...
            if os.path.isdir(os.path.join(root_dir, n)):
                for trial in os.listdir(os.path.join(root_dir, n)):
                    ...
                    for nav_point_idx in range(len(traj_data["navigation_point"])):
                        ...
                        img_feat = [os.path.join(root_dir, n, trial, 'objectsfeatures', str(nav_point).zfill(9) + '_' + str(x) + '.npz.npy') for x in range(4, 12)]
    ...
    def __getitem__(self, idx):
        ...
        resnet_feat = [np.load(x) for x in resnet_feat]
        ...
        for i in range(4):
            ...
            all_img_feats.append(resnet_feat[i])
            all_img_feats.append(resnet_feat[i+4])
        ...
        for act in actseq:
            if act == 'start':
                actseq_list.append(42)
            else:
                direction, distance = act.split('_')
                distance = int(float(distance) // 0.25)
                if direction == 'left':
                    actseq_list.append(43)
                elif direction == 'front':
                    actseq_list.append(44)
                elif direction == 'right':
                    actseq_list.append(45)
                elif direction == 'back':
                    actseq_list.append(46)
                actseq_list.append(distance)
        ...
        return input_ids, all_img_feats, view_idx_lists, actseq_list, trg_direction, trg_distance
